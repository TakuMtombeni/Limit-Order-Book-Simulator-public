# Time dependent Spread with concave Liquidity Taking
import numpy as np
...
class Model2:
    ...
    def gen_market_order(self, lob):
        ...
        if trade_sign == 1:  # make Pv dependent on best_ask_volume
            mu = beta * np.sqrt(lob.bestask_volume())
        else:  # make Pv dependent on best_bid_volume
            mu = beta * np.sqrt(lob.bestbid_volume())
    ...
    def gen_limit_order(self, lob):
        ...
        delta01 = alpha1*math.exp(-alpha2*lob.time)
